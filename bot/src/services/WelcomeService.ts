...
export class WelcomeService {
    ...
    private readonly COLOR_PRIMARY = '#FF6B35'; // Pomora Orange
    private readonly COLOR_SECONDARY = '#001F54'; // Dark Blue
     ...
     */
    async handleGuildCreate(channel: TextChannel) {
        if (!channel) return;
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                { name: '🚀 Quick Start', value: 'Use `/config` to set up your study and report channels.' },
                { name: '👋 Welcomes', value: 'I can also greet new members! Set it up with `/config welcome`.' },
                { name: '📈 Analytics', value: 'I track study time automatically when members join the study voice channel.' }
            )
    ...
    async handleMemberJoin(member: GuildMember, isTest = false) {
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: 'Get Started', value: 'Join a Voice Channel to start your first study session!', inline: true }
            )
            .setTimestamp()
