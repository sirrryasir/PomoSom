    ...
    if (command === 'setup' || command === 'config') {
        ...
        if (subCommand === 'debug') {
            ...
            const stats = new EmbedBuilder()
                ...
                .addFields({ name: 'Configured VC', value: config?.study_channel_id ? `<#${config.study_channel_id}> (${config.study_channel_id})` : 'None' }, { name: 'Current Room State', value: room ? `Running: ${room.isRunning}, Stage: ${room.type}` : 'No Session Found' }, { name: 'Participants (In Memory)', value: participantList });
